...
describe("A tag tree", () => {
    ...
    it("should have a legible string representation", () => {
        const tree = TagTree.fromAnyItem(Tag.DerivationCodeSequence).thenItem(Tag.DerivationCodeSequence, 3)
            .thenAnyItem(Tag.DerivationCodeSequence).thenTag(Tag.PatientID);
    ...
    it("should support string representations with keywords instead of tag numbers where possible", () => {
        const tree = TagTree.fromAnyItem(Tag.DerivationCodeSequence).thenItem(0x11110100, 3)
            .thenAnyItem(Tag.DetectorInformationSequence).thenTag(Tag.PatientID);
        assert.strictEqual(tree.toNamedString(true),
...
describe("Two tag trees", () => {
    ...
    it("should support equals documentation examples", () => {
        ...
        assert(!TagTree.fromAnyItem(0x00089215).thenTag(0x00100010).isEqualTo(TagTree.fromItem(0x00089215, 1)
            .thenTag(0x00100010)));
        assert(TagTree.fromItem(0x00089215, 3).thenTag(0x00100010).isEqualTo(TagTree.fromItem(0x00089215, 3)
            .thenTag(0x00100010)));
    });
});
...
describe("The isTrunkOf test", () => {
    ...
    it("should support documentation examples", () => {
        assert(TagTree.fromItem(0x00089215, 1).thenTag(0x00100010).isTrunkOf(TagPath.fromItem(0x00089215, 1)
            .thenTag(0x00100010)));
        assert(TagTree.fromAnyItem(0x00089215).thenTag(0x00100010).isTrunkOf(TagPath.fromItem(0x00089215, 1)
            .thenTag(0x00100010)));
...
describe("Creating a tag path from a tag tree", () => {
    ...
    it("should handle deep paths", () => {
        assert(TagTree.fromPath(TagPath.fromItem(1, 1).thenItem(2, 2).thenTag(3)).isEqualTo(TagTree.fromItem(1, 1)
            .thenItem(2, 2).thenTag(3)));
    });
...
describe("Parsing a tag tree", () => {
    ...
    it("should work for deep tag trees", () => {
        assert(TagTree.parse("(0008,9215)[*].(0008,9215)[666].(0010,0010)")
            .isEqualTo(TagTree.fromAnyItem(Tag.DerivationCodeSequence).thenItem(Tag.DerivationCodeSequence, 666)
