...
describe('A tag path', () => {
    it('should have a legible string representation', () => {
        const path = TagPath.fromItem(Tag.DerivationCodeSequence, 4)
    ...
    it('should support string representations with keywords instead of tag numbers where possible', () => {
        const path = TagPath.fromItem(Tag.DerivationCodeSequence, 1)
            .thenItem(0x11110100, 3)
            .thenItem(Tag.DetectorInformationSequence, 3)
            .thenTag(Tag.PatientID);
        assert.strictEqual(
            path.toNamedString(true),
...
describe('Comparing two tag paths', () => {
    ...
    it('should establish a total ordering among tag paths encountered when parsing a file', () => {
        ...
        const sorted = createPaths()
            .reverse()
            .sort((a, b) => (a.isBelow(b) ? -1 : 1));
...
describe('Two tag paths', () => {
    ...
    it('should should support equals documentation examples', () => {
        ...
        assert(
            TagPath.fromItem(0x00089215, 3)
                .thenTag(0x00100010)
                .isEqualTo(TagPath.fromItem(0x00089215, 3).thenTag(0x00100010)),
        );
    });
});
...
describe('Parsing a tag path', () => {
    ...
    it('should work for deep tag paths', () => {
        assert(
            TagPath.parse('(0008,9215)[1].(0008,9215)[666].(0010,0010)').isEqualTo(
