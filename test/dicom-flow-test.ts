...
describe("The dicom flow", () => {
    it("should call the correct events for streamed dicom parts", () => {
        ...
        const testFlow = createFlow(new class extends IdentityFlow {
            ...
            public onSequenceDelimitation() { return [new util.TestPart("Sequence End")]; }
            public onItemDelimitation() { return [new util.TestPart("Item End")]; }
            public onItem() { return [new util.TestPart("Item Start")]; }
            public onSequence() { return [new util.TestPart("Sequence Start")]; }
            public onValueChunk() { return [new util.TestPart("Value Chunk")]; }
            public onDeflatedChunk(): DicomPart[] { return []; }
            public onUnknown(): DicomPart[] { return []; }
            public onPart(): DicomPart[] { return []; }
        }());
...
describe("The start event flow", () => {
    ...
    it("should call onStart once for flows with more than one capability using the onStart event", () => {
        ...
        return util.streamPromise(
            singleSource(dicomEndMarker, true), testFlow, arraySink((parts) => {
                assert.strictEqual(parts.length, 1);
                assert.strictEqual(parts[0], dicomEndMarker);
            }));
...
describe("The InSequence support", () => {
    it("should keep track of sequence depth", () => {
        ...
        const check = (depth: number, inSequence: boolean) => {
...
describe("DICOM flows with tag path tracking", () => {
    it("should update the tag path through attributes, sequences and fragments", () => {
        ...
        const expectedPaths = [
            ...
            TagPath.fromItem(Tag.EnergyWindowInformationSequence, 2)
                .thenItem(Tag.EnergyWindowRangeSequence, 1).thenTag(Tag.StudyDate), // Study date value
            TagPath.fromItem(Tag.EnergyWindowInformationSequence, 2)
