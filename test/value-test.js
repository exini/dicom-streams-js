...
describe("Formatting bytes into multiple strings", function () {
    ...
    it("should throw an exception for null input", function () {
        assert.throws(() => {
            Value.create(null).toStrings(VR.SH);
        });
    });

    it("should split a string according to the DICOM multiple value delimiter", function () {
        assert.deepEqual(Value.create(Buffer.from("one\\two\\three")).toStrings(VR.SH), ["one", "two", "three"]);
    });

    it("should trim any characters at beginning and end", function () {
        assert.deepEqual(Value.create(Buffer.from([0x20, 0x20, 0x20, 0x20, 0x41, 0x41, 0x20, 0x20, 0x20])).toStrings(VR.SH), ["AA"]);
    });

    it("should trim any characters at or below 0x20 at beginning and end of each value", function () {
        assert.deepEqual(Value.create(Buffer.from("  one \\ two \\three  ")).toStrings(VR.SH), ["one", "two", "three"]);
    });
    ...
    it("should split and trim strings with multiple character set encodings", function () {
        ...
        assert.deepEqual(Value.create(nameBytes).toStrings(VR.SH, false, CharacterSets.fromNames(["", "ISO 2022 IR 149"])), ["Hong^Gildong=洪^吉洞=홍^길동"]);
    });
});
...
describe("Formatting bytes into a single string", function () {
    ...
    it("should not split a string with DICOM multiple value delimiters", function () {
        assert.equal(Value.create(Buffer.from("one\\two\\three")).toSingleString(VR.SH), "one\\two\\three");
    });

    it("should trim the string components", function () {
        assert.equal(Value.create(Buffer.from("   one two  ")).toSingleString(VR.SH), "one two");
    });
});
...
describe("A Value", function () {
    it("should support adding additional bytes", function () {
        assert.deepEqual(
            Value.create(Buffer.from("one")).append(Buffer.from(" two")).bytes,
            Value.create(Buffer.from("one two")).bytes);
    });

    it("should support ensuring value has even length by adding padding", function () {
        assert.deepEqual(
            Value.create(Buffer.from([111, 110, 101])).ensurePadding(VR.OB).bytes,
            Buffer.from([111, 110, 101, 0]));
        assert.deepEqual(
            Value.create(Buffer.from([111, 110, 101])).ensurePadding(VR.SH).bytes,
            Buffer.from([111, 110, 101, 32]));
        assert.deepEqual(
            Value.create(Buffer.from([111, 110, 101, 111])).ensurePadding(VR.SH).bytes,
