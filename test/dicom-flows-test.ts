...
describe("The stop tag flow", () => {
    ...
    it("should apply stop tag correctly also when preceded by sequence", () => {
        const bytes = base.concatv(data.studyDate(), data.sequence(Tag.DerivationCodeSequence), base.item(),
            data.pixelData(10), base.itemDelimitation(), base.sequenceDelimitation(), data.patientNameJohnDoe(),
            data.pixelData(100));
...
describe("The DICOM group length discard filter", () => {
    it("should discard group length elements except 0002,0000", () => {
        const groupLength = base.concat(Buffer.from([8, 0, 0, 0, 85, 76, 4, 0]),
            base.intToBytesLE(data.studyDate().length));
...
describe("The tag filter", () => {
    it("should filter elements in sequences", () => {
        const bytes = base.concatv(data.sequence(Tag.DerivationCodeSequence), base.item(), data.studyDate(),
    ...
    it("should filter elements not matching the condition", () => {
        const bytes = base.concatv(data.preamble, data.fmiGroupLength(data.fmiVersion(), data.transferSyntaxUID()),
            data.fmiVersion(), data.transferSyntaxUID(), data.patientNameJohnDoe(), data.studyDate());

        return util.testParts(bytes, pipe(parseFlow(), tagFilter((tagPath) => base.groupNumber(tagPath.tag()) >= 8,
            () => false)), (parts) => {
            util.partProbe(parts)
                .expectHeader(Tag.PatientName)
                .expectValueChunk()
    ...
    it("should filter elements matching the condition", () => {
        ...
        return util.testParts(bytes, pipe(parseFlow(), tagFilter((tagPath) =>
            !base.isFileMetaInformation(tagPath.tag()), () => false)), (parts) => {
            util.partProbe(parts)
                .expectHeader(Tag.StudyDate)
                .expectValueChunk()
                .expectDicomComplete();
...
describe("The blacklist filter", () => {
    it("should block the entire sequence when a sequence tag is on the black list", () => {
        const bytes = base.concatv(data.studyDate(),
            ...
            data.sequence(Tag.AbstractPriorCodeSequence), base.item(), data.patientNameJohnDoe(),
            base.itemDelimitation(), base.sequenceDelimitation(), base.itemDelimitation(), base.sequenceDelimitation(),
...
describe("The FMI group length flow", () => {
    ...
    it("should work also in flows with file meta information only", () => {
        const correctLength = data.transferSyntaxUID().length;
        const bytes = base.concatv(data.preamble, data.transferSyntaxUID()); // missing file meta information g. length
    ...
    it("should work in flows without preamble", () => {
        const correctLength = data.transferSyntaxUID().length;
        const bytes = base.concatv(data.transferSyntaxUID(), data.patientNameJohnDoe());
    ...
    it("should not emit a group length attribute when there is no FMI", () => {
        const bytes = base.concatv(data.preamble, data.patientNameJohnDoe());
    ...
    it("should keep a zero length group length attribute", () => {
        const bytes = base.concatv(data.fmiGroupLength(base.emptyBuffer), data.patientNameJohnDoe());
...
describe("The utf8 flow", () => {
    it("should transform a japanese patient name encoded with multiple character sets to valid utf8", () => {
        const specificCharacterSet = base.concatv(base.tagToBytesLE(Tag.SpecificCharacterSet), Buffer.from("CS"),
            base.shortToBytesLE(0x0010), base.padToEvenLength(Buffer.from("\\ISO 2022 IR 149"), VR.CS));
        const patientName = base.concatv(base.tagToBytesLE(0x00100010), Buffer.from("PN"), base.shortToBytesLE(0x002C),
        ...
        const bytes = base.concat(specificCharacterSet, patientName);
    ...
    it("should set specific character set to ISO_IR 192 (UTF-8)", () => {
        const bytes = base.concatv(base.tagToBytesLE(Tag.SpecificCharacterSet), Buffer.from("CS"),
            base.shortToBytesLE(0x0010), base.padToEvenLength(Buffer.from("\\ISO 2022 IR 149"), VR.CS));
    ...
    it("should transform data contained in sequences", () => {
        ...
        const patientName = base.concatv(base.tagToBytesLE(0x00100010), Buffer.from("PN"), base.shortToBytesLE(0x0008),
            base.padToEvenLength(Buffer.from([0xD4, 0xCF, 0xC0, 0xDE, 0x5E, 0xC0, 0xDB, 0xB3]), VR.PN));

        const bytes = base.concatv(specificCharacterSet, data.sequence(Tag.DerivationCodeSequence), base.item(),
            patientName, base.itemDelimitation(), base.sequenceDelimitation());
    ...
    it("should not transform data with VR that doesn't support non-default encodings", () => {
        const specificCharacterSet = base.concatv(base.tagToBytesLE(Tag.SpecificCharacterSet), Buffer.from("CS"),
            base.shortToBytesLE(0x0010), base.padToEvenLength(Buffer.from("\\ISO 2022 IR 149"), VR.CS));
        const patientNameCS = base.concatv(base.tagToBytesLE(0x00100010), Buffer.from("CS"),
            base.shortToBytesLE(0x0004), base.padToEvenLength(Buffer.from([0xD4, 0xCF, 0xC0, 0xDE]), VR.PN));

        const bytes = base.concat(specificCharacterSet, patientNameCS);
    ...
    it("should not change a file already encoded with ISO_IR 192 (UTF-8)", () => {
        const specificCharacterSet = base.concatv(base.tagToBytesLE(Tag.SpecificCharacterSet), Buffer.from("CS"),
            base.shortToBytesLE(0x000A), Buffer.from("ISO_IR 192"));
        const patientName = base.concatv(base.tagToBytesLE(Tag.PatientName), Buffer.from("PN"),
            base.shortToBytesLE(0x000C), Buffer.from("ABC^ÅÖ^ﾔ"));
        const bytes = base.concat(specificCharacterSet, patientName);

        return util.testParts(bytes, pipe(parseFlow(), toUtf8Flow()), (parts) => {
            const newBytes = parts.map((p) => p.bytes).reduce((b1, b2) => base.concat(b1, b2), base.emptyBuffer);
...
describe("The sequence length filter", () => {
    it("should replace determinate length sequences and items with indeterminate, and insert delimitations", () => {
        const bytes =
            base.concatv(data.sequence(Tag.DerivationCodeSequence, 56), base.item(16), data.studyDate(),
                base.item(), data.studyDate(), base.itemDelimitation(),
                data.sequence(Tag.AbstractPriorCodeSequence), base.item(), data.studyDate(), base.itemDelimitation(),
                base.item(16), data.studyDate(), base.sequenceDelimitation());

        return util.testParts(bytes, pipe(parseFlow(), toIndeterminateLengthSequences()), (parts) => {
            util.partProbe(parts)
                .expectSequence(Tag.DerivationCodeSequence, base.indeterminateLength)
    ...
    it("should handle empty sequences and items", () => {
        ...
        return util.testParts(bytes, pipe(parseFlow(), toIndeterminateLengthSequences()), (parts) => {
            util.partProbe(parts)
                ...
                .expectValueChunk()
                .expectItemDelimitation()
                .expectItem(2, base.indeterminateLength)
                .expectItemDelimitation()
                .expectItem(3, base.indeterminateLength)
                .expectSequence(Tag.DerivationCodeSequence, base.indeterminateLength)
                .expectSequenceDelimitation()
                .expectItemDelimitation()
                .expectSequenceDelimitation()
