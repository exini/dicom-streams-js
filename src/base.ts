...
export function concat(a: Buffer, b: Buffer): Buffer { return Buffer.concat([a, b], a.length + b.length); }
export function concatv(...buffers: Buffer[]): Buffer { return Buffer.concat(buffers); }
export function flatten<T>(array: T[][]): T[] { return [].concat.apply([], array); }
...
export function nameBasedUID(name: string, root: string): string { return toUID(root, uuidv5(name, uuidNamespace)); }
export function randomUID(root: string): string { return toUID(root, uuidv4()); }

export const multiValueDelimiter = "\\";

export const emptyBuffer = Buffer.alloc(0);

export function toUInt32(num: number): number { return num >>> 0; }
export function toInt32(num: number): number { return num >> 0; }
export function shiftLeftUnsigned(num: number, n: number): number { return toUInt32(num << n); }
...
export function bytesToDoubleLE(bytes: Buffer): number { return bytes.readDoubleLE(0); }

export function tagToBytesBE(tag: number): Buffer { return intToBytesBE(tag); }
...
export function trim(s: string): string { return s.replace(/^[\x00-\x20]*/g, "").replace(/[\x00-\x20]*$/g, ""); }
...
export function item(length: number = indeterminateLength, bigEndian: boolean = false): Buffer {
    return length === indeterminateLength ?
        bigEndian ? itemBE : itemLE :
...
export function itemDelimitation(bigEndian: boolean = false): Buffer {
...
export function isFileMetaInformation(tag: number): boolean { return (tag & 0xFFFF0000) === 0x00020000; }
export function isGroupLength(tag: number): boolean { return elementNumber(tag) === 0; }
...
export function createUID(): string { return randomUID(uidRoot); }
