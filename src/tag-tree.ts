...
export class TagTree extends TagPathLike<TagTree> {
    public static fromTag(tag: number): TagTreeTag {
        return emptyTagTree.thenTag(tag);
    }
    public static fromAnyItem(tag: number): TagTreeAnyItem {
        return emptyTagTree.thenAnyItem(tag);
    }
    public static fromItem(tag: number, item: number): TagTreeItem {
        return emptyTagTree.thenItem(tag, item);
    }

    public static fromPath(tagPath: TagPath): TagTree {
        let root: TagTree;
        const p = tagPath.head();
        if (p instanceof TagPathTag) {
            root = TagTree.fromTag(p.tag());
        } else if (p instanceof TagPathItem) {
            root = TagTree.fromItem(p.tag(), p.item);
        } else if (p instanceof TagPathItemEnd) {
            root = TagTree.fromItem(p.tag(), p.item);
        } else if (p instanceof TagPathSequence) {
            root = TagTree.fromAnyItem(p.tag());
        } else if (p instanceof TagPathSequenceEnd) {
            root = TagTree.fromAnyItem(p.tag());
        } else {
            root = emptyTagTree;
        }
        return tagPath
            ...
            .toList()
            .reduce((t, p1) => {
                if (t instanceof TagTreeTrunk && p1 instanceof TagPathTag) {
                    return t.thenTag(p1.tag());
                }
                if (t instanceof TagTreeTrunk && p1 instanceof TagPathItem) {
                    return t.thenItem(p1.tag(), p1.item);
                }
                if (t instanceof TagTreeTrunk && p1 instanceof TagPathItemEnd) {
                    return t.thenItem(p1.tag(), p1.item);
                }
                if (t instanceof TagTreeTrunk && p1 instanceof TagPathSequence) {
                    return t.thenAnyItem(p1.tag());
                }
                if (t instanceof TagTreeTrunk && p1 instanceof TagPathSequenceEnd) {
                    return t.thenAnyItem(p1.tag());
                }
                return t;
            }, root);
    ...
    public static parse(str: string): TagTree {
        const isSeq = (s: string): boolean => s[s.length - 1] === ']';
        ...
        const createTag = (s: string): TagTreeTag => TagTree.fromTag(parseTag(s));
        const addTag = (s: string, path: TagTreeTrunk): TagTreeTag => path.thenTag(parseTag(s));
    ...
    public previous(): TagTreeTrunk {
        return this.previousVal;
    }

    public isEmpty(): boolean {
        return this === (emptyTagTree as TagTree);
    }
    ...
    public hasPath(tagPath: TagPath): boolean {
        ...
        if (this instanceof TagTreeItem && 'item' in tagPath) {
            return (
                this.item === (tagPath as ItemPath).item &&
        ...
        if (this instanceof TagTreeAnyItem && 'item' in tagPath) {
            return (
                this.tag() === (tagPath as ItemPath).tag() && this.previous().hasPath((tagPath as ItemPath).previous())
            );
        }
    ...
    public hasTrunk(tagPath: TagPath): boolean {
        if (this.depth() >= tagPath.depth()) {
            ...
            for (let i = 0; i < Math.min(thisList.length, thatList.length); i++) {
                ...
                } else if (t instanceof TagTreeItem && p instanceof TagPathSequence) {
                    check = t.tag() === p.tag();
                } else if (t instanceof TagTreeItem && p instanceof TagPathSequenceEnd) {
                    check = t.tag() === p.tag();
                } else if (t instanceof TagTreeAnyItem && (p as ItemPath).item !== undefined) {
                    check = t.tag() === p.tag();
                } else if (t instanceof TagTreeAnyItem && p instanceof TagPathSequence) {
                    check = t.tag() === p.tag();
                } else if (t instanceof TagTreeAnyItem && p instanceof TagPathSequenceEnd) {
                    check = t.tag() === p.tag();
                } else if (t instanceof TagTreeTag && p instanceof TagPathTag) {
                    check = t.tag() === p.tag();
                }
                if (!check) {
                    return false;
    ...
    public hasTwig(tagPath: TagPath): boolean {
        ...
        } else if (this instanceof TagTreeAnyItem && tagPath instanceof TagPathSequence) {
            check = this.tag() === tagPath.tag();
        } else if (this instanceof TagTreeAnyItem && tagPath instanceof TagPathSequenceEnd) {
            check = this.tag() === tagPath.tag();
        ...
        } else if (this instanceof TagTreeTag && tagPath instanceof TagPathTag) {
            check = this.tag() === tagPath.tag();
        }
    ...
    public drop(n: number): TagTree {
        const dropRec = (tree: TagTree, i: number): TagTree => {
            ...
            if (tree instanceof TagTreeItem) {
                return (t as TagTreeTrunk).thenItem(tree.tag(), tree.item);
            }
            if (tree instanceof TagTreeAnyItem) {
                return (t as TagTreeTrunk).thenAnyItem(tree.tag());
            }
            if (tree instanceof TagTreeTag) {
                return (t as TagTreeTrunk).thenTag(tree.tag());
            }
            return emptyTagTree;
        };
    ...
    public toNamedString(lookup: boolean): string {
        ...
        const toTagTreeString = (tree: TagTree, tail: string): string => {
            const itemIndexSuffix =
                tree instanceof TagTreeAnyItem
                    ? '[*]'
                    : 'item' in tree
                      ? '[' + (tree as unknown as ItemPath).item + ']'
                      : '';
...
export class TagTreeTrunk extends TagTree {
    public thenTag(tag: number): TagTreeTag {
        return new TagTreeTag(tag, this);
    }
    public thenAnyItem(tag: number): TagTreeAnyItem {
        return new TagTreeAnyItem(tag, this);
    }
    public thenItem(tag: number, item: number): TagTreeItem {
