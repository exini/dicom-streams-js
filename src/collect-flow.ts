    ...
    return create(new class extends EndEvent(
        ...
        public onPart(part: DicomPart): DicomPart[] {
            if (this.reachedEnd) {
                ...
                if (part !== valueChunkMarker && part !== sequenceDelimitationPartMarker &&
                ...
                if (part instanceof HeaderPart && (tagCondition(this.tagPath) ||
                    part.tag === Tag.SpecificCharacterSet)) {
                    this.currentElement =
                        new ValueElement(part.tag, part.vr, Value.empty(), part.bigEndian, part.explicitVR);
                    return [];
                }
...
export function collectFromTagPathsFlow(tagPaths: TagPath[], label: string, maxBufferSize?: number) {
    const maxTag = tagPaths.length > 0 ? Math.max(...tagPaths.map((t) => t.head().tag())) : 0;
    const tagCondition = (tagPath: TagPath) => tagPaths.some((tp) => tagPath.startsWith(tp));
    const stopCondition = tagPaths.length > 0 ? ((tagPath: TagPath) =>
        tagPath.isRoot() && tagPath.tag() > maxTag) : (() => true);
