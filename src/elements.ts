...
export class Elements {
    ...
    public static empty(
        ...
        zoneOffset: ZoneId = systemZone): Elements {
    ...
    public toStrings(indent: string): string[] {
        ...
        return flatten(this.data.map((e) => {
            if (e instanceof ValueElement) {
                ...
                return [indent + tagToString(e.tag) + space + e.vr.name + space + s + space + space1(s) + " # " +
                    space2(e.length) + space + e.length + ", " + vm + space + Lookup.keywordOf(e.tag)];
            }

            if (e instanceof Sequence) {
                const hDescription = e.length === indeterminateLength ? "Sequence with indeterminate length" :
                    "Sequence with explicit length " + e.length;
                const heading = indent + tagToString(e.tag) + " SQ " + hDescription + space +
                    space1(hDescription) + " # " + space2(toInt32(e.length)) + space + toInt32(e.length) +
                    ", 1 " + Lookup.keywordOf(e.tag);
                const items = flatten(e.items.map((i) => {
                    const iDescription = i.indeterminate ? "Item with indeterminate length" :
                        "Item with explicit length " + i.length;
                    const itemHeading = indent + "  " + tagToString(Tag.Item) + " na " + iDescription + space +
                        space1(iDescription) + " # " + space2(toInt32(i.length)) + space + toInt32(i.length) +
                        ", 1 Item";
                    const elems = i.elements.toStrings(indent + "    ");
                    const itemDelimitation = indent + "  " + tagToString(Tag.ItemDelimitationItem) + " na " +
                        space.repeat(41) + " #     0, 0 ItemDelimitationItem";
                    elems.unshift(itemHeading);
                    elems.push(itemDelimitation);
                    return elems;
                }));
                const delimitation = indent + tagToString(Tag.SequenceDelimitationItem) + " na " +
            ...
            if (e instanceof Fragments) {
                ...
                const heading = indent + tagToString(e.tag) + space + e.vr.name + space + hDescription + space +
                ...
                if (e.offsets !== undefined) {
                    ...
                    offsets = [indent + space + space + tagToString(Tag.Item) + " na " + description + space +
                ...
                const fragments = e.fragments.map((f) => {
                    ...
                    return indent + space + space + tagToString(Tag.Item) + " na " + description + space +
...
class PreambleElement extends Element {
    ...
    public toBytes(): Buffer { return concat(Buffer.from(new Array(128).fill(0)), Buffer.from("DICM")); }
...
export class ValueElement extends ElementSet {
    ...
    public toBytes(): Buffer { return this.toParts().map((p) => p.bytes).reduce(concat); }
    ...
    public toString(): string {
        ...
        const s = strings.join(multiValueDelimiter);
        const vm = strings.length + "";
        return "ValueElement(" + tagToString(this.tag) + " " + this.vr.name + " [" + s + "] # " +
...
export class SequenceElement extends Element {
    ...
    public toString(): string {
        return "SequenceElement(" + tagToString(this.tag) + " SQ # " + this.length + " " +
...
export class FragmentsElement extends Element {
    ...
    public toParts(): DicomPart[] {
        return [new FragmentsPart(this.tag, indeterminateLength, this.vr, this.bigEndian, this.explicitVR,
            HeaderPart.create(this.tag, this.vr, indeterminateLength, this.bigEndian, this.explicitVR).bytes)];
    }
    public toString(): string {
        return "FragmentsElement(" + tagToString(this.tag) + " " + this.vr.name + " # " +
            Lookup.keywordOf(this.tag) + ")";
    }
}
...
export class Sequence extends ElementSet {
    ...
    public toBytes(): Buffer { return this.toElements().map((e) => e.toBytes()).reduce(concat, emptyBuffer); }
    ...
    public toString(): string {
        return "Sequence(" + tagToString(this.tag) + " SQ # " + this.length + " " + this.size + " " +
            Lookup.keywordOf(this.tag) + ")";
    }
}
...
export class Item {
    ...
    public toBytes(): Buffer { return this.toElements(1).map((e) => e.toBytes()).reduce(concat); }
...
export class Fragments extends ElementSet {
    ...
    public toBytes() { return this.toElements().map((e) => e.toBytes()).reduce(concat); }
    public toElements(): Element[] {
        const elements: Element[] = [];
        ...
        if (this.offsets !== undefined) {
            elements.push(
                new FragmentElement(
                    ...
                    new Value(
                        this.offsets.map((offset) => intToBytes(offset, this.bigEndian), this.bigEndian)
